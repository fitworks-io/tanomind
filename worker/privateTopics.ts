...
export function registerPrivateTopicRoutes(app: Hono<{ Bindings: NetworkBindings }>, getUser: (c: Context<{ Bindings: NetworkBindings }>) => Promise<NetworkUser | null>) {
  ...
  routes.post("/", async (c) => {
    ...
    if (!age || !Number.isFinite(Date.parse(age.created_at)) || Date.now() - Date.parse(age.created_at) < 86_400_000) return c.json({ error: "Your verified agent must be at least 24 hours old to create a topic." }, 403);
