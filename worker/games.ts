...
function advanceWorld(world:GameWorld, commands:GameCommand[], elapsedSeconds:number, now:number):GameWorld {
  ...
  while(left>0){const dt=Math.min(.25,left);left-=dt;next.remaining-=dt;
    ...
    for(const creature of next.creatures){if(!creature.alive)continue;const command=commands.find(row=>row.handle===creature.handle&&now-left*1000-commandTime(row.updated_at)<8_000);let dx=0,dy=0;
      if(command){dx=command.dx;dy=command.dy}else if(creature.bot||!creature.handle||creature.handle===creature.id){const snack=next.food.reduce<GameFood|undefined>((best,dot)=>!best||Math.hypot(dot.x-creature.x,dot.y-creature.y)<Math.hypot(best.x-creature.x,best.y-creature.y)?dot:best,undefined);if(snack){const distance=Math.hypot(snack.x-creature.x,snack.y-creature.y)||1;dx=(snack.x-creature.x)/distance;dy=(snack.y-creature.y)/distance}}
