...
export function DotEcosystemGame({embedded=false}:{embedded?:boolean}={}) {
  ...
  const content=<div className={`${embedded?"bg-transparent":"min-h-screen bg-[#070514]"} font-mono text-white`}>
    ...
    <div className={`grid gap-5 ${embedded?"p-0":"p-3 sm:p-5"}`}><section>
      <div ref={fullscreenRef} className={`grid place-items-center bg-[#070514] ${fullscreen?"fixed inset-0 z-[100] h-[100dvh] w-screen overflow-hidden p-0":"relative w-full"}`}>
      <div ref={arenaRef} style={fullscreen&&fullscreenWidth?{width:fullscreenWidth}:undefined} className="server-game-arena relative aspect-[9/16] w-full max-w-[405px] shrink-0 overflow-hidden border-4 border-[#7734e7] bg-[#030209] shadow-[0_0_0_4px_#241b4b,8px_8px_0_#000]" role="application" aria-label="Dot ecosystem game arena">
        ...
        {showLeaderboard?<div className="absolute inset-0 z-50 flex flex-col bg-[#070514]/60 p-5 text-white"><div className="flex items-center gap-2 text-[#fff36b] [text-shadow:2px_2px_0_#000]"><Trophy size={18}/><h2 className="text-sm font-black uppercase tracking-[.15em]">Longest survivors</h2></div><p className="mt-1 text-[8px] uppercase tracking-[.18em] text-white [text-shadow:1px_1px_0_#000]">One point for every second alive</p><ol className="mt-4 min-h-0 flex-1 space-y-1.5 overflow-y-auto">{sorted.slice(0,10).map((agent,index)=><li key={agent.id} className="grid grid-cols-[1.5rem_1fr_auto] items-center gap-2 border border-[#614e9b] bg-[#090616]/65 px-2 py-2 shadow-[2px_2px_0_#000]"><span className={index===0?"text-center text-xs font-black text-[#fff36b]":"text-center text-xs font-black text-[#c7bdf4]"}>{index+1}</span><strong className="flex min-w-0 items-center gap-2 truncate text-[10px] uppercase tracking-wider"><i className="size-2 shrink-0 rounded-full" style={{backgroundColor:agent.color}}/>{agent.name}</strong><strong className="text-[10px] text-[#65f6ff]">{agent.best} pts</strong></li>)}</ol><button onClick={()=>setShowLeaderboard(false)} className="mt-4 w-full border-2 border-[#4bea72] bg-[#10281a]/80 px-4 py-3 text-xs font-black uppercase tracking-[.2em] text-[#4bea72] shadow-[4px_4px_0_#000]">Go live</button></div>:null}
