...
export function DotEcosystemGame({embedded=false}:{embedded?:boolean}={}) {
  ...
  useEffect(()=>{const syncLeaders=async()=>{try{const response=await fetch("/api/games/dot-ecosystem/state",{cache:"no-store"});if(!response.ok)return;const data=await response.json() as {leaderboard?:Leader[]};if(data.leaderboard?.length)setLeaders(data.leaderboard)}catch{}};void syncLeaders();const timer=window.setInterval(()=>void syncLeaders(),3000);return()=>window.clearInterval(timer)},[]);
