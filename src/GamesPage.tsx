  ...
  const content=<div className={`${embedded?"bg-transparent":"min-h-screen bg-[#070514]"} font-mono text-white`}>
    ...
    <div className={`grid gap-5 ${embedded?"p-0":"p-3 sm:p-5"}`}><section>
      ...
      <div ref={arenaRef} style={fullscreen&&fullscreenWidth?{width:fullscreenWidth}:undefined} className="server-game-arena relative aspect-[9/16] w-full max-w-[405px] shrink-0 overflow-hidden border-4 border-[#7734e7] bg-[#030209] shadow-[0_0_0_4px_#241b4b,8px_8px_0_#000]" role="application" aria-label="Dot ecosystem game arena">
        ...
        {food.map(dot=><i key={dot.id} className="absolute rounded-full shadow-[0_0_6px_currentColor] transition-[left,top] duration-500 ease-linear" style={{left:`${dot.x}%`,top:`${dot.y}%`,width:`${dot.size*2.4}px`,height:`${dot.size*2.4}px`,backgroundColor:dot.color,color:dot.color}}/>)}
        ...
        {showLeaderboard?<a href="https://fitworks.io" target="_blank" rel="noreferrer" className="absolute inset-x-0 top-[82px] z-[60] text-center text-[7px] font-bold uppercase tracking-[.2em] text-[#9b91c7] [text-shadow:1px_1px_0_#000]">Sponsored by <span className="text-[#65f6ff] underline decoration-[#65f6ff]/50 underline-offset-2">FITWORKS.IO</span></a>:null}
        {showLeaderboard?<div className="absolute inset-0 z-50 flex flex-col bg-[#070514]/60 p-5 text-white"><div className="mb-4 flex w-full items-center gap-3 border-y border-[#65f6ff]/70 py-3 [text-shadow:2px_2px_0_#000]" aria-label="Dot Ecosystem"><i className="h-px min-w-0 flex-1 bg-gradient-to-r from-transparent to-[#7734e7]"/><span className="relative grid size-9 shrink-0 place-items-center"><i className="absolute size-9 rounded-full border-2 border-[#7734e7]"/><i className="absolute size-5 rounded-full border-2 border-[#65f6ff]"/><i className="size-2 rounded-full bg-[#fff36b] shadow-[0_0_8px_#fff36b]"/></span><span className="shrink-0 text-center"><strong className="block text-sm font-black uppercase tracking-[.2em] text-[#fff36b]">Dot Ecosystem</strong><small className="block text-[7px] uppercase tracking-[.28em] text-[#65f6ff]">Eat · Grow · Survive</small></span><i className="h-px min-w-0 flex-1 bg-gradient-to-l from-transparent to-[#7734e7]"/></div><p className="text-center text-[8px] uppercase tracking-[.18em] text-white [text-shadow:1px_1px_0_#000]">Longest single life · spawn to death</p><ol className="mt-4 min-h-0 flex-1 space-y-1.5 overflow-y-auto">{sorted.slice(0,10).map((agent,index)=><li key={agent.id} className="grid grid-cols-[1.5rem_1fr_auto] items-center gap-2 border border-[#614e9b] bg-[#090616]/65 px-2 py-2 shadow-[2px_2px_0_#000]"><span className={index===0?"text-center text-xs font-black text-[#fff36b]":"text-center text-xs font-black text-[#c7bdf4]"}>{index+1}</span><strong className="flex min-w-0 items-center gap-2 truncate text-[10px] uppercase tracking-wider"><i className="size-2 shrink-0 rounded-full" style={{backgroundColor:agent.color}}/>{agent.name}</strong><strong className="text-[10px] text-[#65f6ff]">{formatSurvivalTime(agent.best)}</strong></li>)}</ol><button onClick={()=>setShowLeaderboard(false)} className="mt-4 w-full border-2 border-[#4bea72] bg-[#10281a]/80 px-4 py-3 text-xs font-black uppercase tracking-[.2em] text-[#4bea72] shadow-[4px_4px_0_#000]">Watch live</button></div>:null}
