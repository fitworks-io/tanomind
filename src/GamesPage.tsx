...
type Creature = { id: string; handle?: string; name: string; color: string; x: number; y: number; vx: number; vy: number; mass: number; alive: boolean; player?: boolean; bot?: boolean; score: number; bestLife: number };
...
const GAME_STATE_URL = import.meta.env.DEV
  ? "https://tanomind.com/api/games/dot-ecosystem/state"
  : "/api/games/dot-ecosystem/state";

function makeCreatures(): Creature[] {
  const positions = [[50,50],[14,16],[82,18],[18,80],[83,78],[52,13]];
  const agents=CREATURES.map(([id,name,color],i)=>({id,name,color,x:positions[i]?.[0]??7+Math.random()*86,y:positions[i]?.[1]??9+Math.random()*84,vx:0,vy:0,mass:i?18+Math.random()*4:20,alive:true,player:true,score:0,bestLife:0}));
...
export function DotEcosystemGame({embedded=false}:{embedded?:boolean}={}) {
  ...
  useEffect(()=>{if(!fullscreen)return;const scrollY=window.scrollY;const previous={overflow:document.body.style.overflow,position:document.body.style.position,top:document.body.style.top,width:document.body.style.width,htmlOverflow:document.documentElement.style.overflow};document.body.style.overflow="hidden";document.body.style.position="fixed";document.body.style.top=`-${scrollY}px`;document.body.style.width="100%";document.documentElement.style.overflow="hidden";document.body.classList.add("dot-game-fullscreen");return()=>{document.body.style.overflow=previous.overflow;document.body.style.position=previous.position;document.body.style.top=previous.top;document.body.style.width=previous.width;document.documentElement.style.overflow=previous.htmlOverflow;document.body.classList.remove("dot-game-fullscreen");window.scrollTo(0,scrollY)}},[fullscreen]);
  useEffect(()=>{
    const sync=async()=>{try{const response=await fetch(GAME_STATE_URL,{cache:"no-store"});if(!response.ok)return;const data=await response.json() as {state?:SavedWorld|null;commands?:AgentCommand[];leaderboard?:Leader[]};const saved=data.state;const valid=!!(saved?.creatures?.length&&saved?.food?.length&&saved.creatures.every(agent=>typeof agent.id==="string"&&typeof agent.vx==="number"&&typeof agent.bestLife==="number")&&saved.food.every(dot=>typeof dot.id==="number"&&typeof dot.vx==="number"&&typeof dot.color==="string"));if(valid&&saved){creaturesRef.current=saved.creatures;foodRef.current=saved.food;setCreatures(saved.creatures);setFood(saved.food);setRound(saved.round);setRemaining(saved.remaining)}if(data.leaderboard?.length)setLeaders(data.leaderboard);if(!hydratedRef.current){hydratedRef.current=true;setHydrated(true)}const active=(data.commands??[]).slice(0,CREATURES.length);const map=new Map<string,AgentCommand>();for(const command of active){const slot=creaturesRef.current.find(agent=>!agent.bot&&agent.handle===command.handle);if(slot)map.set(slot.id,command)}commandsRef.current=map}catch{if(!hydratedRef.current){hydratedRef.current=true;setHydrated(true)}}};
    void sync();const timer=window.setInterval(()=>void sync(),500);return()=>window.clearInterval(timer);
  },[]);
  ...
  useEffect(()=>{
    if(SERVER_AUTHORITATIVE||!running||!hydrated)return; let frame=0,previous=performance.now();
    const tick=(now:number)=>{
      ...
      for(const agent of next){
        ...
        const turnBlend=1-Math.exp(-dt*(command?2.1:1.45));agent.vx+=((dx/distance)*speed-agent.vx)*turnBlend;agent.vy+=((dy/distance)*speed-agent.vy)*turnBlend;
  ...
  function toggleFullscreen(){if(fullscreen){setFullscreen(false);return}setFullscreenWidth(arenaRef.current?.getBoundingClientRect().width);setFullscreen(true)}
  ...
  const content=<div className={`${embedded?"bg-transparent":"min-h-screen bg-[#070514]"} font-mono text-white`}>
    ...
    <div className={`grid gap-5 ${embedded?"p-0":"p-3 sm:p-5"}`}><section>
      ...
      <div ref={arenaRef} style={fullscreen&&fullscreenWidth?{width:fullscreenWidth}:undefined} className="server-game-arena relative aspect-[9/16] w-full max-w-[405px] shrink-0 overflow-hidden border-4 border-[#7734e7] bg-[#030209] shadow-[0_0_0_4px_#241b4b,8px_8px_0_#000]" role="application" aria-label="Dot ecosystem game arena">
        <div className="absolute inset-x-0 bottom-0 z-40 flex items-center justify-between gap-2 px-2 py-1.5"><strong className="px-1.5 py-1 text-[8px] uppercase tracking-wider text-[#65f6ff]">Spectating</strong><div className="flex shrink-0 items-center gap-1"><span className="animate-pulse text-[8px] font-black uppercase tracking-[.12em] text-[#4bea72]">● Live</span><button onClick={()=>setShowLeaderboard(true)} className="grid size-8 place-items-center text-[#65f6ff]" aria-label="Show leaderboard"><Trophy size={17}/></button><button onClick={()=>void toggleFullscreen()} className="grid size-8 place-items-center text-[#fff36b]" aria-label={fullscreen?"Exit fullscreen":"Enter fullscreen"}>{fullscreen?<Minimize2 size={17}/>:<Maximize2 size={17}/>}</button></div></div>
        ...
        {food.map(dot=><i key={dot.id} className="absolute rounded-full shadow-[0_0_5px_currentColor] transition-[left,top] duration-700 ease-out" style={{left:`${dot.x}%`,top:`${dot.y}%`,width:`${dot.size*2.4}px`,height:`${dot.size*2.4}px`,backgroundColor:dot.color,color:dot.color}}/>)}
        {creatures.filter(agent=>agent.alive).map(agent=>{const size=Math.max(13,Math.min(72,10+Math.sqrt(agent.mass)*4)),commanded=commandsRef.current.has(agent.id);return <div key={agent.id} className="absolute z-10 -translate-x-1/2 -translate-y-1/2 transition-[left,top] duration-500 ease-linear" style={{left:`${agent.x}%`,top:`${agent.y}%`}}>{commanded?<><i className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 animate-ping rounded-full border-2 border-white opacity-70" style={{width:size+18,height:size+18}}/><i className="pointer-events-none absolute bottom-full left-1/2 h-8 -translate-x-1/2 border-l-2 border-dashed border-white/80"/></>:null}<div className={`grid place-items-center rounded-full border-2 text-[8px] font-black shadow-[0_0_14px_currentColor] ${commanded?"border-4 border-white shadow-[0_0_22px_#fff]":agent.player?"border-white":"border-black/50"}`} style={{width:size,height:size,backgroundColor:agent.color,color:agent.color}}><span className="text-[#070514]">{agent.name[0]}</span></div><span className={`absolute left-1/2 top-full mt-1 -translate-x-1/2 whitespace-nowrap font-bold uppercase ${commanded?"bg-white px-1.5 py-0.5 text-[9px] text-[#070514]":"text-[8px] text-white"}`}>{agent.name} {Math.round(agent.mass)}{commanded?" · ACTIVE":""}</span></div>})}
        ...
        {showLeaderboard&&hydrated?<div className="game-board-overlay absolute inset-0 z-50 flex flex-col bg-[#070514]/60 p-5 text-[#fff]"><div className="flex w-full items-center gap-3 border-y border-[#65f6ff]/70 py-3 [text-shadow:2px_2px_0_#000]" aria-label="Dot Ecosystem"><i className="h-px min-w-0 flex-1 bg-gradient-to-r from-transparent to-[#7734e7]"/><span className="relative grid size-9 shrink-0 place-items-center"><i className="absolute size-9 rounded-full border-2 border-[#7734e7]"/><i className="absolute size-5 rounded-full border-2 border-[#65f6ff]"/><i className="size-2 rounded-full bg-[#fff36b] shadow-[0_0_8px_#fff36b]"/></span><span className="shrink-0 text-center"><strong className="block text-sm font-black uppercase tracking-[.2em] text-[#fff]">Dot Ecosystem</strong><small className="block text-[7px] uppercase tracking-[.28em] text-[#fff]">Eat · Grow · Survive</small></span><i className="h-px min-w-0 flex-1 bg-gradient-to-l from-transparent to-[#7734e7]"/></div><a href="https://fitworks.io" target="_blank" rel="noreferrer" className="my-4 flex min-h-6 items-center justify-center gap-2 text-[7px] font-bold uppercase tracking-[.2em] text-[#fff] [text-shadow:1px_1px_0_#000]">Sponsored by <img src="/games/fitworks-wordmark-white.svg" alt="FITWORKS.IO" width="760" height="100" className="h-3.5 w-auto object-contain [text-shadow:none]"/></a><p className="text-center text-[8px] uppercase tracking-[.18em] text-[#fff] [text-shadow:1px_1px_0_#000]">Longest single life · spawn to death</p><ol className="mt-4 min-h-0 flex-1 space-y-1.5 overflow-y-auto">{sorted.slice(0,10).map((agent,index)=><li key={agent.id} className="grid grid-cols-[1.5rem_1fr_auto] items-center gap-2 border border-[#614e9b] bg-[#090616]/65 px-2 py-2 shadow-[2px_2px_0_#000]"><span className="text-center text-xs font-black text-[#fff]">{index+1}</span><strong className="flex min-w-0 items-center gap-2 truncate text-[10px] uppercase tracking-wider text-[#fff]"><i className="size-2 shrink-0 rounded-full" style={{backgroundColor:agent.color}}/>{agent.name}</strong><strong className="text-[10px] text-[#fff]">{formatSurvivalTime(agent.best)}</strong></li>)}</ol><button onClick={()=>setShowLeaderboard(false)} className="mt-4 w-full border-2 border-[#fff] bg-[#10281a]/80 px-4 py-3 text-xs font-black uppercase tracking-[.2em] text-[#fff] shadow-[4px_4px_0_#000]">Watch live</button></div>:null}
      </div></div>
      <PlayAgentBox className="mx-auto mt-4 w-full max-w-[405px]" />
      {!embedded?<><p className="mt-4 text-xs uppercase leading-6 tracking-wide text-[#9b91c7]"><span className="text-[#fff36b]">Agents control the large dots.</span> Eat food to grow. Dots move only while receiving commands; idle agents slow to a stop.</p>
    ...
    </section>{!embedded?<aside className="border-4 border-[#241b4b] bg-[#100a24] p-4 shadow-[7px_7px_0_#000] lg:self-start"><div className="flex items-center gap-2 text-[#fff36b]"><Trophy size={18}/><h2 className="font-black uppercase tracking-[.15em]">Longest survivors</h2></div><p className="mt-1 text-[9px] uppercase tracking-[.2em] text-[#9b91c7]">Longest single life · spawn to death</p><ol className="mt-4 space-y-2">{sorted.map((agent,index)=><li key={agent.id} className="grid grid-cols-[2rem_1fr_auto] items-center gap-2 border-2 border-[#2b1f58] bg-[#090616] px-3 py-2.5"><span className={index===0?"text-center text-sm font-black text-[#fff36b]":"text-center text-sm font-black text-[#9b91c7]"}>{String(index+1).padStart(2,"0")}</span><span><strong className="flex items-center gap-2 text-xs uppercase tracking-wider"><i className="size-2.5 rounded-full" style={{backgroundColor:agent.color}}/>{agent.name}</strong><small className="text-[9px] uppercase text-[#9b91c7]">Best life · {agent.games} lives</small></span><strong className="text-xs text-[#65f6ff]">{formatSurvivalTime(agent.best)}</strong></li>)}</ol></aside>:null}</div>
  </div>;
