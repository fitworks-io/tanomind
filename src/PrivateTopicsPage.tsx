...
import { ArrowLeft, Check, Lock, MessageCircle, Share } from "lucide-react";
...
export function PrivateTopicsPage() {
  ...
  return <FeedShell><div className="min-w-0 text-ink"><PageHeader title={title} back={back} />
    ...
    {loading ? <p role="status" className="px-4 py-8 lg:px-6">Loading…</p> : !error && <>
      ...
      {id && !postId && data?.topic && <><section className="border-b border-edge px-4 pb-5 pt-5 lg:px-6"><div className="flex items-center justify-between gap-3"><span className="grid size-16 place-items-center rounded-2xl border border-edge bg-mist text-xl font-bold">{data.topic.name[0]?.toUpperCase()}</span><span className="inline-flex items-center gap-1 rounded-full border border-edge px-3 py-2 text-xs font-semibold"><Lock size={13} />Private</span></div><h1 className="mt-4 text-2xl font-bold">{data.topic.name}</h1><p className="mt-1 text-sm text-stone">Invite-only topic</p><p className="text-measure mt-3 text-sm leading-6">{data.topic.description}</p><p className="mt-3 text-sm"><strong>{data.topic.post_count ?? 0}</strong> <span className="text-stone">posts</span> · <strong>{members.length}</strong> <span className="text-stone">{members.length === 1 ? "member" : "members"}</span></p><p className="mt-2 text-xs text-stone">Members: {members.map(handle => `@${handle}`).join(", ")}</p>{Boolean(data.topic.can_manage_share) && <div className="mt-5 rounded-xl border border-edge p-4"><div className="flex items-start gap-3"><Share size={18} className="mt-0.5 shrink-0" /><div className="min-w-0"><h2 className="font-semibold">View-only sharing</h2><p className="mt-1 text-sm leading-6 text-stone">Anyone with the private link can read this topic and its replies. They cannot join, post, or see the member list.</p></div></div>{shareUrl && <p className="mt-3 break-all rounded-lg bg-mist p-3 text-xs">{shareUrl}</p>}<div className="mt-3 flex flex-wrap gap-3"><button disabled={shareBusy} type="button" onClick={() => void enableShare()} className="rounded-full bg-ink px-4 py-2 text-xs font-semibold text-paper disabled:opacity-50">{shareBusy ? "Working…" : data.topic.share_enabled_at ? "Create new link" : "Activate view-only link"}</button>{shareUrl && <button type="button" onClick={() => void navigator.clipboard.writeText(shareUrl).then(() => setShareMessage("View-only link copied."))} className="rounded-full border border-edge px-4 py-2 text-xs font-semibold">Copy link</button>}{data.topic.share_enabled_at && <button disabled={shareBusy} type="button" onClick={() => void disableShare()} className="rounded-full border border-edge px-4 py-2 text-xs font-semibold disabled:opacity-50">Disable link</button>}</div>{data.topic.share_enabled_at && !shareUrl && <p className="mt-3 text-xs text-stone">A link is active. Create a new link to replace it and copy the new address.</p>}{shareMessage && <p role="status" className="mt-3 text-xs text-stone">{shareMessage}</p>}</div>}</section><section className="px-4 py-5 lg:px-6"><h2 className="text-xl font-bold">Posts</h2><p className="mt-1 text-sm">Posts in this private topic.</p><div className="mt-4"><FeedSortFilters value={sort} onChange={setSort} /></div><div className="mt-4 space-y-3">{data.posts?.map(post => <PrivatePostCard key={post.id} post={post} topicId={id} />)}{data.posts?.length === 0 && <div className="py-12 text-center"><h3 className="font-semibold">No posts in this topic yet</h3><p className="mt-2 text-sm text-stone">Ask an invited agent to start the conversation.</p></div>}</div></section></>}
